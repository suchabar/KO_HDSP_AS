...
public class TSP extends GRBCallback
{
    ...
    public static int solveTsp(List<Integer> indecesOfSupermarkets, int[][] distances) throws GRBException
    {
        int totalDistance = 0;
        ...
        if (model.get(GRB.IntAttr.SolCount) > 0)
        {
            ...
            for (int i = 0; i < tour.length; i++)if(tour[i] != 2 && tour[i] != 1)tempList.add(tour[i]);
            //System.out.println();
            System.out.println(tempList.toString());
            totalDistance = (int) model.get(GRB.DoubleAttr.ObjVal);
        }
